...
PropertyMacro* FContainerMeta::GetBuiltinProperty(BuiltinType type)
{
    ...
    if (type < MaxBuiltinType)
    {
        ...
        switch (type)
        {
            ...
            case TBool:
                Ret = new FBoolProperty(PropertyMetaRoot, NAME_None);
                static_cast<FBoolProperty*>(Ret)->SetBoolSize(1, true, 0xFF);
                break;
            case TByte:
                Ret = new FByteProperty(PropertyMetaRoot, NAME_None);
                Ret->PropertyFlags |= CPF_HasGetValueTypeHash;
                break;
            case TInt:
                Ret = new FIntProperty(PropertyMetaRoot, NAME_None);
                Ret->PropertyFlags |= CPF_HasGetValueTypeHash;
                break;
            case TFloat:
                Ret = new FFloatProperty(PropertyMetaRoot, NAME_None);
                Ret->PropertyFlags |= CPF_HasGetValueTypeHash;
                break;
            case TDouble:
                Ret = new FDoubleProperty(PropertyMetaRoot, NAME_None);
                Ret->PropertyFlags |= CPF_HasGetValueTypeHash;
                break;
            case TInt64:
                Ret = new FInt64Property(PropertyMetaRoot, NAME_None);
                Ret->PropertyFlags |= CPF_HasGetValueTypeHash;
                break;
            case TString:
                Ret = new FStrProperty(PropertyMetaRoot, NAME_None);
                Ret->PropertyFlags |= CPF_HasGetValueTypeHash;
                break;
            case TText:
                Ret = new FTextProperty(PropertyMetaRoot, NAME_None);
                Ret->PropertyFlags |= CPF_HasGetValueTypeHash;
                break;
            case TName:
                Ret = new FNameProperty(PropertyMetaRoot, NAME_None);
                Ret->PropertyFlags |= CPF_HasGetValueTypeHash;
                break;
#else
            case TBool:
...
#elif ENGINE_MINOR_VERSION > 0 && ENGINE_MAJOR_VERSION > 4
        Ret = new FObjectProperty(PropertyMetaRoot, NAME_None);
...
#elif ENGINE_MINOR_VERSION > 0 && ENGINE_MAJOR_VERSION > 4
        Ret = new FStructProperty(PropertyMetaRoot, NAME_None);
    ...
    else if (auto Enum = Cast<UEnum>(Field))
    {
        ...
        if (Enum->GetCppForm() == UEnum::ECppForm::EnumClass)
        {
            FEnumProperty* EnumProp =
                ...
                new FEnumProperty(PropertyMetaRoot, NAME_None);
            EnumProp->SetEnum(Enum);
#else
                new FEnumProperty(PropertyMetaRoot, NAME_None, RF_Transient, 0, CPF_HasGetValueTypeHash, Enum);
#endif
            FNumericProperty* UnderlyingProp = new FByteProperty(EnumProp, TEXT("UnderlyingType"));
            EnumProp->AddCppProperty(UnderlyingProp);
#if ENGINE_MINOR_VERSION >= 5 && ENGINE_MAJOR_VERSION >= 5
        ...
        {
            FByteProperty* ByteProp = new FByteProperty(PropertyMetaRoot, NAME_None);
            ByteProp->Enum = Enum;
